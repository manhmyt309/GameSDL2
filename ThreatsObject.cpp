...
void ThreatsObject::SetBulletRect(const int& idx)
{
	...
	if (x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
	{
		if (y_val_ > 0) {
			...
			int val2 = map_data.tile[y2][x2];
				...
				if ((val1 != BLANK_TILE && val1 != STATE_MONEY) || (val2 != BLANK_TILE && val1 != STATE_MONEY))
				{
					y_pos_ = y2 * TILE_SIZE;
		...
		else if (y_val_ < 0)
		{
			...
			int val2 = map_data.tile[y1][x2];
				...
				if ((val1 != BLANK_TILE && val1 != STATE_MONEY) ||( val1 != BLANK_TILE && val1 != STATE_MONEY))
				{
...
void ThreatsObject::ImpMoveType(SDL_Renderer* screen)
{
	...
	else
	{
		...
		/*	if (rect_.x >= 0 && abs(rect_.x - main_object_position.x) < 800)
			{
...
void ThreatsObject::InitBullet(BulletObject* p_bullet, SDL_Renderer* screen)
{
	if (p_bullet != NULL)
	{
		 p_bullet->set_bullet_type(BulletObject::ARROW_BULLET);
